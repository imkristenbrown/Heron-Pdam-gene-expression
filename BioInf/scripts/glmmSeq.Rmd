...
coldata$Treatment_chr <- coldata$Treatment
coldata$Treatment <- gsub("Variable", 10, coldata$Treatment)
coldata$Treatment <- gsub("Stable", 5, coldata$Treatment)
coldata$Treatment <- as.numeric(coldata$Treatment)
head(coldata)
```
...
dds <- DESeq(dds)
dispersions <- setNames(dispersions(dds), rownames(cts_raw))
...
```{r}
sizeFactors <- estimateSizeFactorsForMatrix(cts_raw)
```
...
> gene expression ~ Treatment * Origin + (1 | Colony)
...
saveRDS(results, file = "raw_fit.rds")
```

Showed errors in 9527 genes (out of 33730), meaning that 24203 worked!

### Outputs
```{r}
results <- readRDS(file = "raw_fit.rds")
...
kable(stats[order(stats[, 'P_Treatment:Origin']), ]) %>%
  kable_styling() %>%
...
```{r}
summary(results, gene = "Pocillopora_acuta_HIv2___RNAseq.g21448.t1")
```
...
```{r}
results <- glmmQvals(results) # adds a matrix to the results object containign Q-vals
```
...
## Alternative: Using filtered data instead of raw counts as input
...
Summary stats for a specific gene.
```{r}
summary(results_filt, gene = "Pocillopora_acuta_HIv2___RNAseq.g13538.t2")
```
