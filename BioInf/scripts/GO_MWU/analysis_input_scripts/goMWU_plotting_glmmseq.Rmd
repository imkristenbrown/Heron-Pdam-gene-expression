---
title: "glmmseq GOMWU Analysis - Plotting"
author: "Zoe Dellaert"
date: "7/22/2024"
...
```{r}
all_tests <- c("DE_Origin", "DE_Origin_upSlope", "DE_Origin_upFlat", "DE_Treatment", "DE_Interaction",
               "frontloaded") 
```
...
for (test in all_tests){
  print(test)
  file_name <- paste0("MWU_BP_", test)

  data <- read.table(paste0("../../../output/glmmseq/GO_MWU/",file_name,".csv"), header = TRUE)
  ...
  ggsave(paste0("../../../output/glmmseq/GO_MWU/", test,"_filtp05.pdf"), freq_fig, width = 4, height = 5, units = c("in"), dpi=300)
  ...
  freq_fig <- data %>% dplyr::filter(p.adj<0.01) %>%
    ...
    scale_x_discrete(labels = label_wrap(30)) +
    labs(title = paste0("Enriched Biological Process GO Terms,\n DE Genes by ", test), x="Term", y="Number of Significant (p.adj < 0.01)\nGO Terms", colour="p-value") +
    theme_classic() + 
    theme(axis.text.y = element_text(size = 8), 
              axis.title = element_text(size = 8), 
              plot.title = element_text(hjust = 0.5, size = 8))
  ...
  ggsave(paste0("../../../output/glmmseq/GO_MWU/", test,"_filtp01.pdf"), freq_fig, width = 4, height = 5, units = c("in"), dpi=300)
  ...
  print(paste0("The p.adj < 0.05 terms enriched by GO_MWU in the ",test," test have been reduced to ", length(unique(data_reduced$term)), " terms, with ",length(unique(data_reduced$name)), " unique term descriptions, and belonging to ", length(unique(data_reduced$ParentTerm)), " unique parent terms."))
  ...
  scale_y_continuous(limits=c(0,13))+
   scale_x_discrete(labels = label_wrap(30)) +
  labs(title = paste0("Enriched Biological Process Parent Terms,\n DE Genes by ", test), x="Parent Term", y="Number of Significant (p.adj < 0.05)\nGO Terms", colour="p-value") +
  theme_classic() + 
  theme(axis.text.y = element_text(size = 8), 
              axis.title = element_text(size = 8), 
              plot.title = element_text(hjust = 0.5, size = 8))
  ...
  ggsave(paste0("../../../output/glmmseq/GO_MWU/Parent_", test,"_filtp05.pdf"), parent_fig, width = 4, height = 5, units = c("in"), dpi=300)
