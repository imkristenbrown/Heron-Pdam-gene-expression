...
origin_signif <- rownames(heatmappval[heatmappval$Origin < 0.05,]) 

colony_signif <- rownames(heatmappval[heatmappval$Colony < 0.05,])

# Calc up mods:
calc_up <- rownames(moduleTraitCor_both[moduleTraitCor_both$Calcification.per.day > 0,])

calc_up <- calc_up[calc_up %in% calc_signif & calc_up %in% origin_signif & calc_up %in% colony_signif]

# Calc down mods:
calc_down <- rownames(moduleTraitCor_both[moduleTraitCor_both$Calcification.per.day < 0,])

calc_down <- calc_down[calc_down %in% calc_signif & calc_down %in% origin_signif & calc_down %in% colony_signif]
```
