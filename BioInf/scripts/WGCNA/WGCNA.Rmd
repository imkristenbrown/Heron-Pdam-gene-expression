...
This is a Rmd file analyzing our raw count data by the WGCNA package as described in the [manual](https://cran.r-project.org/web/packages/WGCNA/WGCNA.pdf) and tutorials.
...
## Individual Module Plots, with n = number of genes in module

Make a vector of calcification- and origin-associated modules to explore in detail:

We are using a more stringent significance cutoff for origin (p < 0.01).


```{r}
moduleTraitCor_both <- as.data.frame(moduleTraitCor_both)
heatmappval <- as.data.frame(heatmappval)

# Modules significant by calcification

calc_signif <- rownames(heatmappval[heatmappval$Calcification.per.day < 0.05,])

origin_signif <- rownames(heatmappval[heatmappval$Origin < 0.01,]) 

density_signif <- rownames(heatmappval[heatmappval$CaCO3.Density..g.ml. < 0.05,])

# Calc up mods:
calc_up <- rownames(moduleTraitCor_both[moduleTraitCor_both$Calcification.per.day > 0,])

calc_up <- calc_up[calc_up %in% calc_signif & calc_up %in% origin_signif & calc_up %in% density_signif]

# Calc down mods:
calc_down <- rownames(moduleTraitCor_both[moduleTraitCor_both$Calcification.per.day < 0,])

calc_down <- calc_down[calc_down %in% calc_signif & calc_down %in% origin_signif & calc_down %in% density_signif]
```

```{r}
unique(geneInfo_GROUPS$moduleColor)
...
```{r}
for (color in calc_up) {
  module_name <- gsub("ME", "", color)
  n_mod <- nrow(geneInfo_GROUPS %>% dplyr::filter(moduleColor == module_name))
  
  pdf(paste0("../../output/WGCNA/plot_MEs_fig_Trt_Orig_", toupper(module_name), ".pdf"))
  
  plot_MEs_fig_Trt_Orig <- ggplot(filter(plot_MEs, Module == color), aes(y=Mean, x=Treatment, color=Origin, fill=Origin)) + 
    ...
    geom_hline(yintercept = 0, linetype="dashed", color = 'black', size=0.7, show.legend = TRUE) +
    scale_fill_manual("Origin", values=c("Slope"='#26519B', "Flat"= "#FE180C")) +
    scale_color_manual("Origin", values=c("Slope"='#26519B', "Flat"= "#FE180C")) +
    scale_y_continuous(expression(Mean~module~eigenegene), limits=c(-0.4, 0.4), expand = c(0.005, 0.005)) +  
    theme_classic() +
    labs(title = paste0(toupper(substring(module_name, 1, 1)), substring(module_name, 2), " Module")) +
