...
---
#Gene expression - revised March 2023
```{r}
#rm(list = ls())
```
...
```{r}
cts_raw2<-read.csv("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/HeronPdam_gene_count_matrix.csv")
#as.matrix(cts_raw)
#cts_raw$gene_id <- factor(cts_raw$gene_id)
cts_raw <- cts_raw2[,-1]
rownames(cts_raw) <- cts_raw2[,1]
cts_raw
```
```{r}
...
```{r}
coldata <- read.csv("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/RNA Submission Sample List metadata.csv")
coldata <- coldata[,c("Sample.Name","Origin","Treatment")]
#coldata$Sample.Name <- as.factor(coldata$Sample.Name)
...
sum(cts_filtpoa)
#as.matrix(cts_filtpoa)
```

```{r}
#identify genes to keep by count filter
gkeep <- cts_filt[cts_filtpoa,]
#identify gene lists
gn.keep <- rownames(gkeep)
#gene count data filtered in PoverA, P percent of the samples have counts over A
gcount_filt <- as.data.frame(cts_filt[which(rownames(cts_filt) %in% gn.keep),])
#How many rows do we have before and after filtering?
nrow(cts_filt) #Before
...
##Removing outliers RF16A and RF16C
```{r}
cts_out <-dplyr::select(gcount_filt, -c('RF16A', 'RF16C'))#here we remove the two samples that are behaving weirdly
coldata_out <-coldata[-c(7, 8),]#here we remove the two samples that are behaving weirdly
```


#Filtering 
```{r}
cts_filt_2<-cts_out[rowSums(!as.matrix(cts_out)) < ncol(cts_out), ]#here we remove all genes that were not expressed in any of our samples- we are left with 24,333 genes.
cts_filt_2
```

```{r}
write.csv(cts_filt_2, "Counts filtered.csv")
```
...
library(genefilter)
ffun<-filterfun(pOverA(0.25,10)) 
cts_filtpoa<- genefilter((cts_filt_2), ffun)
sum(cts_filtpoa)
#as.matrix(cts_filtpoa)
```
...
```{r}
#identify genes to keep by count filter
gkeep <- cts_filt_2[cts_filtpoa,]
#identify gene lists
gn.keep <- rownames(gkeep)
#gene count data filtered in PoverA, P percent of the samples have counts over A
gcount_filt <- as.data.frame(cts_filt_2[which(rownames(cts_filt_2) %in% gn.keep),])
#How many rows do we have before and after filtering?
nrow(cts_filt_2) #Before
nrow(gcount_filt) #After
```
```{r}
library("DESeq2")
dds2 <- DESeqDataSetFromMatrix(countData = gcount_filt,
                              colData=coldata_out,
                              design= ~Origin+Treatment)
dds2
```
...
# png to output 
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/sft.png", 1000, 1000, pointsize=20)
par(mfrow = c(1,2));
cex1 = 0.9;
...
sizeGrWindow(8,6)
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/GeneDendrogram.png", 1000, 1000, pointsize=20)
...
# Plot the result
sizeGrWindow(7, 6)
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/ClusterEigengenes.png", 1000, 1000, pointsize=20)
...
# Plot the result
sizeGrWindow(7, 6)
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/ClusterEigengenes_merged.png", 1000, 1000, pointsize=20)
...
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/ClusterDendrogram_signed.png", 1000, 1000, pointsize=20)
...
save(MEs, moduleLabels, moduleColors, geneTree_sign, file = "/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/networkConstruction-stepByStep.RData")
# write csv - save the module eigengenes
write.csv(MEs, file = "/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/WGCNA_ModulEigengenes.csv")
table(mergedColors)
```
...
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/ClusterTree_Origin.png", 1000, 1000, pointsize=20)
...
# save data
save(vst2, Traits, file = "/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/dataInput.RData")

# write the vst transformed data 
write.csv(vst2, "/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/vstTransformed_WGCNAdata.csv") # write
```
...
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/Treatments_Primary_heatmap2.png", 500, 1000, pointsize=20)
...
#plot it
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/ME_Boxplot.png", 600, 1000, pointsize=20)
...
```
```{r}
# **Plot mean eigengene over developmental stages**  View module eigengene data and make dataframe for Strader plots.  
head(MEs)
names(MEs)
Strader_MEs <- MEs
Strader_MEs$Origin <- coldata_out$Origin
...
plot_MEs<-Strader_MEs%>%
  gather(., key="Module", value="Mean", MEred:MEtan)
plot_MEs
```
...
```{r,warning=FALSE}
plot_MEs_fig<-ggplot(plot_MEs, aes(y=Mean, x=Origin, color=Origin, fill=Origin))+ 
...
```{r}
d <- d<-read.csv("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/Heron pHi coral physiology and respirometry R_reduced.csv", strip.white=T)
...
# Display the correlation values within a heatmap plot
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/Both_with phys and pHi_heatmap_new.png", 2000, 2000, pointsize=20)
...
#png(file = "/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/Both_with phys and pHi_heatmap_new2.png", height = 2500, width = 2500)
ht=Heatmap(moduleTraitCor_both, name = "Eigengene", column_title = "Module-Trait Eigengene Correlation", 
        ...
        column_order = 1:17, column_dend_reorder = FALSE, cluster_columns = hclust(dist(t(moduleTraitCor_both)), method = "average"),column_dend_height = unit(0.5, "in"),
        ...
        column_names_gp =  gpar(fontsize = 12), row_names_gp = gpar(fontsize = 10, alpha = 0.75, border = TRUE, fill = htmap.colors))
#draw(ht)
ht
```
...
MEsPlotting_melt <- melt(MEsPlotting, id=c('Coral_ID', 'Origin', 'Treatment'))
#plot it
png("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/ME_Boxplot_Treatment.png", 600, 1000, pointsize=20)
ggplot(MEsPlotting_melt, aes(x=Treatment, y=value, fill = factor(Treatment), shape=Treatment)) +
  geom_boxplot(aes(middle = mean(value)), position=position_dodge(0.8), outlier.size = 0, alpha = 0.5) + 
  stat_summary(fun.y = mean, color = "black", position = position_dodge(0.75),
               geom = "point", shape = 19, size = 3,
               show.legend = FALSE) +
...
names(MEs)
Strader_MEs <- MEs
Strader_MEs$Treatment <- coldata_out$Treatment
Strader_MEs$Coral_ID <- rownames(Strader_MEs)
head(Strader_MEs)
Strader_MEs<-Strader_MEs%>%
  droplevels() #drop unused level
...
```{r}
plot_MEs_mean<-summarySE(plot_MEs, measurevar='Mean', groupvars=c('Treatment',"Module"), na.rm=TRUE, conf.interval = 0.95)
plot_MEs_mean
```
####Plot mean module eigengene for each module.  
```{r,warning=FALSE}
plot_MEs_fig2<-ggplot(plot_MEs, aes(y=Mean, x=Treatment, color=Treatment, fill=Treatment))+ 
  geom_point(alpha=0.8,position=position_jitterdodge(0.05))+
  geom_boxplot(alpha=0.4, color="black", outlier.shape = NA)+
  #geom_smooth(method=gam)+
  geom_hline(yintercept = 0, linetype="dashed", color = 'black', size=0.7, show.legend = TRUE)+
...
plot_MEs_fig2<-ggplot(plot_MEs, aes(y=Mean, x=Treatment, color=Origin, fill=Origin))+ 
  geom_point(alpha=0.8,position=position_jitterdodge(0.05))+
  geom_boxplot(alpha=0.4, color="black", outlier.shape = NA)+
  #geom_smooth(method=gam)+
  geom_hline(yintercept = 0, linetype="dashed", color = 'black', size=0.7, show.legend = TRUE)+
...
```{r}
eggnog<-read.delim("/Users/imkri/Desktop/Penn Postdoc/Heron experiment/Pocillopora_acuta_HIv2.genes.EggNog_results.txt")
...
```{r}
write.csv(geneInfo_GROUPS, file = "/Users/imkri/Desktop/Penn Postdoc/Heron experiment/R figures/WGCNA_ModulMembership.csv")
```