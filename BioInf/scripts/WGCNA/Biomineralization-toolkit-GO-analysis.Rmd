...
```{r}
go_results_BP_up_reduced <- go_results_BP_up_reduced %>%
   filter(grepl(paste(keywords, collapse="|"), ParentTerm))

go_results_BP_down_reduced <- go_results_BP_down_reduced %>%
   filter(grepl(paste(keywords, collapse="|"), ParentTerm))

go_results_BP_biomin_reduced <- go_results_BP_biomin_reduced %>%
   filter(grepl(paste(keywords, collapse="|"), ParentTerm))
```
...
```{r}
cal_biomin_terms <- read.csv("../../output/WGCNA/GO_analysis/goseq_pattern_biomin_filtered_keywords.csv")
