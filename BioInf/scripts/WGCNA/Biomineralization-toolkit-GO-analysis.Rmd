...
## Plot of parent terms SHARED _and_ UNIQUE for Up and Down modules, Biomin only (this is basically redundant! Removes a few terms only)
...
merged_up <- parent_filtered_up %>%
  ...
  mutate(
    ...
    Sum_of_UpGOterms = sum(UpGOTerms, na.rm = TRUE),
    Proportion.of.shared.GO.terms.with.biomineralization.genes = case_when(
      Factor %in% c("Up, Biomin", "Down, Up, Biomin") ~ UpGOTerms / Number.of.terms,
      TRUE ~ 0
    )
  ) %>%
  mutate(Percentage.of.shared.GO.terms.with.biomineralization.genes = Proportion.of.shared.GO.terms.with.biomineralization.genes * 100) %>%
...
merged_down <- parent_filtered_down %>%
  ...
  mutate(
    ...
    Sum_of_DownGOterms = sum(DownGOTerms, na.rm = TRUE),
    Proportion.of.shared.GO.terms.with.biomineralization.genes = case_when(
      Factor %in% c("Down, Biomin", "Down, Up, Biomin") ~ DownGOTerms / Number.of.terms,
      TRUE ~ 0
    )
  ) %>%
  mutate(Percentage.of.shared.GO.terms.with.biomineralization.genes = Proportion.of.shared.GO.terms.with.biomineralization.genes * 100) %>%
...
ggsave(filename="../../output/WGCNA/GO_analysis/freq_fig_up_colored_all_biomin.pdf", plot=freq_fig_up, dpi=300, height=25, width=10, units="in", limitsize=FALSE)
```
...
ggsave(filename="../../output/WGCNA/GO_analysis/freq_fig_down_colored_all_biomin.pdf", plot=freq_fig_down, dpi=300, height=25, width=10, units="in", limitsize=FALSE)
```
...
ggsave(filename="../../output/WGCNA/GO_analysis/freq_fig_up_colored_all_biomin_gr20.pdf", plot=freq_fig_up, dpi=300, height=10, width=10, units="in", limitsize=FALSE)
```
...
ggsave(filename="../../output/WGCNA/GO_analysis/freq_fig_down_colored_all_biomin_gr20.pdf", plot=freq_fig_down, dpi=300, height=10, width=10, units="in", limitsize=FALSE)
```