...
calc_down_mods <- c("turquoise","magenta","lightcyan","tan")
...
nrow(geneInfo %>% filter(moduleColor=="tan")) #170

sum(nrow(geneInfo %>% dplyr::filter(moduleColor=="turquoise")), nrow(geneInfo %>% dplyr::filter(moduleColor=="magenta")), nrow(geneInfo %>% filter(moduleColor=="lightcyan")),nrow(geneInfo %>% filter(moduleColor=="tan")))

# 3012

other_mods <- c("green","yellow", "purple", "midnightblue","cyan")

sum(nrow(geneInfo %>% dplyr::filter(moduleColor=="green")), nrow(geneInfo %>% dplyr::filter(moduleColor=="yellow")), nrow(geneInfo %>% filter(moduleColor=="purple")), nrow(geneInfo %>% filter(moduleColor=="midnightblue")), nrow(geneInfo %>% filter(moduleColor=="cyan")))

# 1874

# 4126 + 3012 + 1874 = 9012, which represents all of our genes
```
...
3012 genes are in the 4 modules significantly downregulated by calcification.
...
### Generate vector with names in just the module we are analyzing
ID.vector <- geneInfo%>%
  filter(moduleColor %in% c("turquoise","magenta","lightcyan","tan"))%>%
  pull(gene_id)

length(ID.vector) #3012
...
freq_fig_up<-ggplot(cal_freq_terms_filtered_up_all, aes(y=Number.of.terms,x=reorder(ParentTerm, Number.of.terms), fill=Percentage.of.shared.GO.terms.with.biomineralization.genes,group=1))+
  ...
  geom_hline(yintercept = 0, linetype="solid", color = 'black', size=0.5, show.legend = TRUE)+
  coord_flip()+
  scale_y_continuous(expression(GO~term~counts),limits=c(0,100))+
  #scale_color_manual("Direction.of.flat.origin",values= c("up"="#b2182b","no pattern"="grey","down" ="#67a9cf"))+
  #scale_fill_manual("Direction.of.flat.origin",values= c("up"="#b2182b","no pattern"="grey","down" ="#67a9cf"))+
  scale_fill_gradientn(colours=c("white","#fddbc7","#f4a582","#d6604d","#b2182b"), na.value = "grey98",limits = c(0, 100))+ 
 #scale_color_gradientn(colours=c("#b2182b","#fddbc7","white","#d1e5f0","#67a9cf", "#67a9cf", "#2166ac"), na.value = "grey98",limits = c(-0, 40))+ 
...
```{r}
nrow(geneInfo %>% dplyr::filter(moduleColor=="green")) #563

### Generate vector with names in just the module we are analyzing
ID.vector <- geneInfo%>%
  filter(moduleColor %in% "green")%>%
  #get_rows(.data[[module]]))%>%
  pull(gene_id)
...
#Write file of results 
write.csv(GO, file = "../../output/WGCNA/GO_analysis/goseq_pattern_green.csv")
```

```{r}
go_results <-read.csv("../../output/WGCNA/GO_analysis/goseq_pattern_green.csv")
```
...
```{r}
#go_results<-go_results %>% filter(grepl(paste(keywords, collapse="|"), ParentTerm))
```
...
##Construct list of genes with 1 for genes in module and 0 for genes not in the module
    ...
    #Write file of results 
    write.csv(KO, "../../output/WGCNA/GO_analysis/goseq_KEGG_pattern_green.csv")
    
```

```{r}
ko_results_green<-read.csv("../../output/WGCNA/GO_analysis/goseq_KEGG_pattern_green.csv") 
head(ko_results_green)
```
