...
goterms_shared_full <- goterms_shared %>% left_join(dplyr::select(all_terms,GOterm, Factor, bh_adjust), by = "GOterm") %>%
  mutate(bh_adjust_Up = ifelse(Factor.y == "Up", bh_adjust, NA)) %>%
  mutate(bh_adjust_Down = ifelse(Factor.y == "Down", bh_adjust, NA)) %>%
  dplyr::select(-c("bh_adjust", "Factor.y")) %>%
  group_by(GOterm, ParentTerm, Factor.x, term) %>%
  dplyr::summarize(bh_adjust_Up = na.omit(bh_adjust_Up)[1],
            bh_adjust_Down = na.omit(bh_adjust_Down)[1])
```
