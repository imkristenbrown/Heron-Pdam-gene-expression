...
fixed_ylim_maPlot <- function (object, x1var, x2var, x1Values = NULL, x2Values = NULL, 
          pCutoff = 0.01, plotCutoff = 1, zeroCountCutoff = 50, colours = c("grey", 
                                                                            "midnightblue", "mediumvioletred", "goldenrod"), labels = c(), 
          fontSize = 12, labelFontSize = 4, useAdjusted = FALSE, graphics = "ggplot", 
          verbose = FALSE) 
{
  ...
  plotData <- data.frame(cbind(predict[, paste0("y_", outLabels)], 
  ...
  yCols <- modelData$y[modelData[, x2var] == x2Values[2] & 
                         modelData[, x1var] %in% x1Values]
  plotData$x <- log2(plotData[, xCols[1]] + 1) - log2(plotData[, 
                                                               xCols[2]] + 1)
  plotData$y <- log2(plotData[, yCols[1]] + 1) - log2(plotData[, 
                                                               yCols[2]] + 1)
  ...
  colLevels <- c("Not Significant", paste0(adj, x1var, " < ", 
                                           pCutoff), paste0(adj, x2var, " < ", pCutoff, " (up in ", 
                                                            x2Values[2], ")"), paste0(adj, x2var, " < ", pCutoff, 
                                                                                      " (up in ", x2Values[1], ")"))
  ...
  plotData$col[plotData$col == paste0(adj, x2var, " < ", pCutoff, 
                                      " (up in ", x2Values[2], ")") & plotData$maxGroup == 
  ...
  plotData$zerocount <- rowSums(object@countdata[rownames(object@predict), 
  ] < 1)
  plotGenes <- apply(plotData[, grep(adj, colnames(plotData))], 
                     1, function(x) {
                       any(x < plotCutoff)
                     })
    ...
    annot1 <- lapply(labels, function(i) {
      ...
      list(x = x, y = y, text = i, textangle = 0, ax = x/z * 
             75, ay = -y/z * 75, font = list(color = "black", 
                                             size = 12), arrowcolor = "black", arrowwidth = 1, 
           arrowhead = 0, arrowsize = 1.5, xanchor = "auto", 
           yanchor = "auto")
    })
  ...
  if (graphics == "ggplot") {
      ...
      labs(x = bquote(paste("Mean log"[2], "(gene expression + 1)")), 
           y = yLab, title = paste0("MA plot (", x2var, 
                                    " = ", x2Values[1], ")")) + geom_hline(yintercept = 0, 
                                                                           colour = "grey60", linetype = "dashed") + theme(legend.position = c(1, 
                                                                                                                                               0), text = element_text(size = fontSize), axis.text = element_text(colour = "black", 
                                                                                                                                                                                                                  size = fontSize - 1), legend.background = element_rect(fill = NA, 
                                                                                                                                                                                                                                                                         color = NA), legend.justification = c(1.1, -0.1)) + 
      annotate("text", x = unlist(lapply(annot1, function(x) x$x)), 
               y = unlist(lapply(annot1, function(x) x$y)), 
               vjust = 1.3, size = labelFontSize, label = unlist(lapply(annot1, 
                                                                        function(x) x$text)))
    ma2 <- ggplot(data = plotData, aes_string(x = "meanexp", 
      ...
      annotate("text", x = unlist(lapply(annot2, function(x) x$x)), 
               y = unlist(lapply(annot2, function(x) x$y)), 
               vjust = 1.3, size = labelFontSize, label = unlist(lapply(annot2, 
                                                                        function(x) x$text)))
    combined <- ggarrange(ma1, ma2, ncol = 1, nrow = 2, 
                          common.legend = TRUE)
  }
  else if (graphics == "plotly") {
    ma1 <- plot_ly(data = plotData, x = ~meanexp, y = ~x, 
                   type = "scatter", mode = "markers", color = ~col, 
                   colors = colours, marker = list(size = 8, line = list(width = 0.5, 
                                                                         color = "white")), text = rownames(plotData), 
                   hoverinfo = "text") %>% layout(title = paste0("MA plot (", 
                                                                 x2var, " = ", x2Values[1], ")"), annotations = annot1, 
                                                  font = list(size = fontSize), xaxis = list(title = "Mean log2 gene expr + 1", 
                                                                                             color = "black"), yaxis = list(title = yLab, 
                                                                                                                            color = "black"), legend = list(x = 0.65, y = 0.04, 
                                                                                                                                                            font = list(color = "black")))
    ma1_dummy <- plot_ly(data = plotData, x = ~meanexp, 
                         y = ~x, type = "scatter", mode = "markers", color = ~col, 
                         colors = colours, marker = list(size = 8, line = list(width = 0.5, 
                                                                               color = "white")), text = rownames(plotData), 
                         hoverinfo = "text", showlegend = FALSE) %>% layout(title = paste0("MA plot (", 
                                                                                           x2var, " = ", x2Values[1], ")"), annotations = annot1, 
                                                                            xaxis = list(title = "Mean log2 gene expr + 1", 
                                                                                         color = "black"), yaxis = list(title = yLab, 
                                                                                                                        color = "black"), font = list(size = fontSize), 
    ...
    ma2 <- plot_ly(data = plotData, x = ~meanexp, y = ~y, 
                   ...
                   colors = colours, marker = list(size = 8, line = list(width = 0.5, 
                                                                         color = "white")), text = rownames(plotData), 
                   hoverinfo = "text") %>% layout(title = paste0("MA plot (", 
                                                                 x2var, " = ", x2Values[2], ")"), annotations = annot2, 
                                                  font = list(size = fontSize), xaxis = list(title = "Mean log2 gene expr + 1", 
                                                                                             color = "black"), yaxis = list(title = yLab, 
                                                                                                                            color = "black"), legend = list(x = 0.65, y = 0.04, 
                                                                                                                                                            font = list(color = "black")))
    combined <- plotly::subplot(ma1_dummy, ma2 %>% layout(title = "MA Plots"), 
                                nrows = 2, shareY = TRUE, shareX = FALSE, margin = 0.1) %>% 
      layout(annotations = list(list(x = 0, y = 0.45, 
                                     text = x2Values[1], showarrow = FALSE, xref = "paper", 
                                     yref = "paper", font = list(size = 15)), list(x = 0, 
                                                                                   y = 1.01, text = x2Values[2], showarrow = FALSE, 
                                                                                   xref = "paper", yref = "paper", font = list(size = 15))), 
             font = list(size = fontSize), legend = list(x = 1, 
                                                         y = 0.5, xanchor = "right", yanchor = "center"))
  }
  else {
    stop("graphics must be either ggplot or plotly.")
  }
