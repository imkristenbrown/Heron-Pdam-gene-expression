---
title: "Reverse Frontloading Analysis after glmmSeq (Flat to Slope), Slope as Base Level"
author: "Zoe Dellaert"
date: "5/25/2023"
...
#REVERSING THE LEVELS HERE FROM FLAT TO SLOPE FOR CURIOUSITY
```{r}
...
#for our purposes, conditioned = Slope and naive = Flat, Ambient = Stable, and Moderate = Variable


# X axis - this is the relative fold ratio of the conditioned-control to the naive-control as the following: 
# [ (Variable_Slope/Stable_Slope) / (Variable_Flat/Stable_Flat) ]
...
xall_1 <- ( (READY$y_Variable_Slope / READY$y_Stable_Slope) / (READY$y_Variable_Flat / READY$y_Stable_Flat) ) # call y_Stable_Slope as the control for the y_Variable_Slope ratio

xall_2 <- ( (READY$y_Variable_Slope / READY$y_Stable_Flat) / (READY$y_Variable_Flat / READY$y_Stable_Flat) ) # call y_Stable_Flat as the control for the y_Variable_Slope ratio

xall_3 <- (READY$y_Variable_Slope / READY$y_Variable_Flat)  # call y_Stable_Flat as the control for the y_Variable_Slope ratio
...
# lets calculate it 
yall <- (READY$y_Stable_Slope / READY$y_Stable_Flat)
...
P_allgenes <- READY %>% #dplyr::filter(yall < 6) %>% 
        ...
        labs(y= "Slope to Flat (Conditioned to naive) control ratio", 
             x = "Slope to Flat (Conditioned to naive) foldchange ratio",
...
3680 genes are frontloaded, with a yall ratio larger than 1 and a xall_1 ratio less than 1.
