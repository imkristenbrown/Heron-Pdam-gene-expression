...
```{r}
### Generate vector with names of the 11 significant DEGs by Origin that are upregulated in the Slope origin in both treatments
ID.vector_updownSlope <- geneInfo %>%
  filter(q_Origin < 0.05) %>%
  filter(Stable_OriginFC > 0 & Variable_OriginFC < 0) %>%
  pull(gene_id)
...
gene.vector_updownSlope = as.integer(ALL.vector %in% ID.vector_updownSlope) 

names(gene.vector_updownSlope) <- ALL.vector #set names

#weight gene vector by bias for length of gene
pwf_updownSlope <- nullp(gene.vector_updownSlope, ID.vector_updownSlope, bias.data = LENGTH.vector) 
```

```{r}
#run goseq using Wallenius method for all categories of GO terms 
GO.wall_updownSlope <- goseq(pwf_updownSlope, ID.vector_updownSlope, gene2cat=GO.terms, method="Wallenius", use_genes_without_cat=TRUE)
...
#Write file of results 
write.csv(GO_updownSlope, "../../../output/Slope_Base/GOSeq/GOseq_DEG_Origin_updownSlope.csv")
...
2 genes: one is unannotated and one is: 
...
ID.vector_downupSlope <- geneInfo %>%
  filter(q_Origin < 0.05) %>%
  filter(Stable_OriginFC < 0 & Variable_OriginFC > 0) %>%
  pull(gene_id)
...
gene.vector_downupSlope = as.integer(ALL.vector %in% ID.vector_downupSlope) 

names(gene.vector_downupSlope) <- ALL.vector #set names

#weight gene vector by bias for length of gene
pwf_downupSlope <- nullp(gene.vector_downupSlope, ID.vector_downupSlope, bias.data = LENGTH.vector) 
```
...
#Write file of results 
write.csv(GO_downupSlope, "../../../output/Slope_Base/GOSeq/GOseq_DEG_Origin_downupSlope.csv")
