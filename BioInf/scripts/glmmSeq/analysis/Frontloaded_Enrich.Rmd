...
pdf(file = "../../../output/glmmseq/Frontloaded_GOSeq/Frontloadeds_P05_GOSEM_kmeans_10max.pdf", width = 7, height = 10)
simplifyGO(Mat, word_cloud_grob_param = list(max_width = 50), method = "kmeans", max_words=10)
dev.off()
```

```{r}
library(simplifyEnrichment)
BP_terms <- GO_01 %>% dplyr::filter(ontology=="BP") %>% dplyr::select(GOterm) %>% unlist()

Mat <- GO_similarity(BP_terms, ont = "BP", db = "org.Ce.eg.db")

pdf(file = "../../../output/glmmseq/Frontloaded_GOSeq/Frontloadeds_P01_GOSEM_kmeans_10max.pdf", width = 7, height = 10)
simplifyGO(Mat, word_cloud_grob_param = list(max_width = 50), method = "kmeans", max_words=10)
dev.off()
```
