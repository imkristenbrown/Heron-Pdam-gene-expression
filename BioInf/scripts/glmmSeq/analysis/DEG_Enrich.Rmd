...
There are 7 over-represented GO terms (by padjust <0.05) in the 346 significant DEGs by Origin that are upregulated in the Slope origin in both treatments
...
There 0 243 over-represented GO terms (by padjust <0.05) in the 481 significant DEGs by Origin that are upregulated in the Flat origin in both treatments
...
#adjust p-values 
GO_updownSlope$bh_adjust <- p.adjust(GO_updownSlope$over_represented_pvalue, method="BH") #add adjusted p-values
...
GO_updownSlope_01 <- GO_updownSlope %>%
        dplyr::filter(over_represented_pvalue<0.01) %>%
        dplyr::arrange(., ontology, over_represented_pvalue)

dim(GO_updownSlope_05)
dim(GO_updownSlope_01)

```

There are 0 over-represented GO terms (by p <0.05) in the 11 significant DEGs by Origin that are upregulated in the Slope origin in Stable treatment and downregulated in the variable treatment
...
#adjust p-values 
GO_downupSlope$bh_adjust <- p.adjust(GO_downupSlope$over_represented_pvalue, method="BH") #add adjusted p-values
...
#Filtering for p < 0.01
GO_downupSlope_01 <- GO_downupSlope %>%
        dplyr::filter(over_represented_pvalue<0.01) %>%
        dplyr::arrange(., ontology, over_represented_pvalue)

dim(GO_downupSlope_05)
dim(GO_downupSlope_01)

```

There are 0 over-represented GO terms (by p <0.05) in the 2 significant DEGs by Origin that are downregulated in the Slope origin in Stable treatment and upregulated in the variable treatment
